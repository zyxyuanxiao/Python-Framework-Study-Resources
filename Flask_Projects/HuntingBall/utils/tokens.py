...
def set_token(token, user):
    r.hmset('user:%s' % user.username, {'token': token})
    r.set('token:%s' % token, user.username)
    r.expire('user:%s' % user.username, 3600 * 24 * 30)
    r.expire('token:%s' % token, 3600 * 24 * 30)

    # tk = str(uuid.uuid1())
    # r.set('login-' + username, tk, ex=3600 * 24 * 30)
    # r.set('login-' + username, tk, ex=30)
